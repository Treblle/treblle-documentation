...
In plain PHP, Treblle is unable to automatically determine the parameterised route path for a request. For example, for `articles/12345` the endpoint path would be `articles/{id}`. Supplying the route pattern lets Treblle build a much more accurate API representation.
...
Attach custom key/value pairs to any request. They appear on the Treblle dashboard and can be used to filter and search requests.
...
<TipCard>
`user-id` is a reserved keyword that powers customer dashboards and tracks usage across your API.
</TipCard>
...
Supply the parameterised route pattern via a PSR-7 request attribute named `_treblle_route_path`:
...
1. **`fastcgi_finish_request()`** (PHP-FPM) — flushes the HTTP response to the client and continues running in the background. The Treblle call is completely invisible to your end users.
...
| `$excludedHeaders` (4th positional arg) | _(removed)_ | Use `masked_keywords` to hide header values instead |
