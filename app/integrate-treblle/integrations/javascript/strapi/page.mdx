...
To integrate Treblle with <Link href="https://strapi.io" target="_blank">Strapi</Link>, we provide an official unified SDK for JavaScript frameworks - <Link href="https://github.com/Treblle/treblle-node" target="_blank">`treblle`</Link>.
    ...
    initialize() {
      strapi.app.use(
        strapiTreblle({
          ...
          ignoreAdminRoutes: ["admin", "content-manager", "upload"], // Optional: Skip Strapi admin routes
        })
      );
    },
  };
...
<InfoTable title="Option" description="Description">
    ...
    <InfoTable.Row
        ...
        description='Array of admin route prefixes to ignore (default: ["admin", "content-type-builder", "content-manager"]). Strapi-specific - helps avoid logging internal admin panel requests that are typically not part of your public API.'
    />
</InfoTable>
...
```js copy
const { strapiTreblle, DEFAULT_MASKED_KEYWORDS } = require("treblle");
