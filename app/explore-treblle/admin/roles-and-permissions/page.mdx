...
description: "Create custom roles with granular permissions for your Enterprise workspace. Manage per-API and environment-specific access with fine-grained controls."
---
...
<ImageCard src="/platform/people/workspace.jpg" alt="Customer workspace view" />
...
<ImageCard src="/platform/people/docs.jpg" alt="Customer documentation interface" />
...
<ImageCard src="/platform/people/requests.jpg" alt="Customer request history filtered by customer ID" />
...
<ImageCard src="/platform/people/dashboard.jpg" alt="Customer dashboard with real-time analytics" />
...
<ImageCard src="/platform/people/metadata.jpg" alt="treblle-metadata header example" />
```javascript
   req.headers['treblle-metadata'] = JSON.stringify({
     'user-id': 'Netflix' // Must be globally unique
   });
```
2. Customer must be invited with the Customer role
3. Customer ID must be globally unique across your workspace
4. Customer ID in invitation should match the ID used in API requests
5. API documentation should be complete and accurate for customer reference
